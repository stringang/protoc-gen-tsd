 ...
 * Usage:
 *
 *  > node bin/generate-methods src/protos/echo.protos > src/echo.interface.ts
 *
...
const contents = fs.readFileSync(protoPath).toString();
...
function generateMethodCalls(node, name) {
  ...
  _.forOwn(node.methods, (method, mname) => {
    ...
    if (method.responseStream && !method.requestStream) {
      ...
      template('request-stream-method', params);
    } else {
      template('unary-method', params);
    }
  });
